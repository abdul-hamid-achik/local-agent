...
func TestToolCardManagerSetDarkUpdatesSpinnerAndPreservesCallID(t *testing.T) {
	...
	assertToolCardForeground(t, "light spinner", mgr.Cards[0].Spinner.Style, "#4f8f8f")
	...
	if !mgr.IsDark || !card.IsDark {
		t.Fatal("manager and card should both use the dark theme")
	}
	assertToolCardForeground(t, "dark spinner", card.Spinner.Style, "#88c0d0")
	assertToolCardForeground(t, "dark running title", card.Styles.TitleRunning, "#88c0d0")
	if card.Spinner.Style.GetBold() {
		t.Fatal("spinner style should remain unbolded")
	}
}
