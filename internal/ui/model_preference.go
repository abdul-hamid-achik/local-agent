...
// manually selected model. It is intentionally independent from session state.
type ModelPreferenceStore interface {
	SetManualModel(string) error
	ClearManualModel() error
}

// SetModelPreferenceStore enables restart persistence for explicit /model and
// picker selections. Startup flags and agent profiles remain separate runtime
// authorities and are never written through this store.
