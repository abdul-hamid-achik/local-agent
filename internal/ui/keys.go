...
func DefaultKeyMap() KeyMap {
	return KeyMap{
		...
		Paste: key.NewBinding(
			key.WithKeys("ctrl+v"),
			key.WithHelp("ctrl+v", "paste text, or a macOS clipboard image as PNG"),
		),
		...
		InspectOutput: key.NewBinding(
			key.WithKeys("alt+o"),
			key.WithHelp("alt+o", "full output (inspected receipt)"),
		),
		InspectDiff: key.NewBinding(
			key.WithKeys("alt+d"),
			key.WithHelp("alt+d", "full diff (inspected receipt)"),
		),
