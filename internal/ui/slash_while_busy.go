...
// iteration.
//
// A local host makes the substrate half of that rule concrete rather than
// theoretical: switching models mid-turn asks Ollama to load new weights while
// the current request is still streaming from the old ones, on a machine whose
