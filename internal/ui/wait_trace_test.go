...
func TestWaitTraceRendersHeadMarkerAndTrack(t *testing.T) {
	...
	// change: on the first frame of a wait the head is still at the origin and
	// eases out, which is what makes it read as motion.
	settle := func() string {
		for range 400 {
			m.advanceWaitTrace()
		}
		return ansi.Strip(m.renderWaitTrace(waitTraceCells))
	}

	// A quarter of the way to the baseline puts the true position at 0.75 of
	// a cell: the head renders on cell 1 and smears back onto cell 0. Two lit
...
func TestWaitTraceSpringGivesTheHeadMomentum(t *testing.T) {
	base := time.Date(2026, 8, 5, 12, 0, 0, 0, time.UTC)
	m := waitTraceTestModel(t, base)
	m.state = StateWaiting
	m.chromeSpring.wait.inWait = true
	m.chromeSpring.wait.waitStart = base
	m.chromeSpring.wait.baseline = 2 * time.Second
	m.chromeSpring.wait.samples = 1
	...
	target := waitTraceTarget(2*time.Second, 2*time.Second, waitTraceCells)
	if target != float64(waitTraceCells/2) {
		t.Fatalf("target at the baseline = %v, want the marker at %d", target, waitTraceCells/2)
	}

	m.advanceWaitTrace()
	first := m.chromeSpring.wait.headPos
	if first <= 0 {
	...
	m.advanceWaitTrace()
	second := m.chromeSpring.wait.headPos
	if second <= first {
	...
	for range 200 {
		m.advanceWaitTrace()
		if m.chromeSpring.wait.headPos > target {
			overshot = true
			break
		}
	}
...
func TestWaitTraceResetsMomentumBetweenRequests(t *testing.T) {
	base := time.Date(2026, 8, 5, 12, 0, 0, 0, time.UTC)
	m := waitTraceTestModel(t, base)
	m.chromeSpring.wait.baseline = time.Second
	m.chromeSpring.wait.samples = 1
	m.chromeSpring.wait.headPos = 4.5
	m.chromeSpring.wait.headVel = 9

	m.state = StateWaiting
	m.observeWaitTrace()
	if m.chromeSpring.wait.headPos != 0 || m.chromeSpring.wait.headVel != 0 {
		t.Fatalf("new request inherited motion: pos=%v vel=%v",
			m.chromeSpring.wait.headPos, m.chromeSpring.wait.headVel)
	}
}
...
func TestWaitTraceSpringIsInertUnderReducedMotion(t *testing.T) {
	base := time.Date(2026, 8, 5, 12, 0, 0, 0, time.UTC)
	m := waitTraceTestModel(t, base)
	m.reducedMotion = true
	m.state = StateWaiting
...
func TestWaitTraceTrailLightsTheVacatedCell(t *testing.T) {
	if cell, weight := waitTraceTrail(3.0, 3); cell != -1 || weight != 0 {
	...
	cell, weight := waitTraceTrail(3.4, 3)
	if cell != 4 {
