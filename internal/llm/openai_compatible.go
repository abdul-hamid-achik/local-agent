...
func (c *OpenAICompatibleClient) consumeSSE(ctx context.Context, body io.ReadCloser, fn func(StreamChunk) error) error {
	...
	// every remote provider. Only the native Ollama client filled it.
	//
	// Time-to-first-token and total duration need no provider cooperation. The
	// per-phase durations do, so they stay zero, which the contract already
	// defines as "not reported" rather than "instant".
...
type openAIChatRequest struct {
	...
	// contract omits usage unless include_usage is set, and Local Agent's
	// budgets are not advisory: a turn that ends without a trustworthy receipt
	// charges its reservation fail-closed, so a provider that follows the spec
	// strictly would exhaust a goal's budget on its first turn.
	//
	// The native Ollama protocol always reports eval counts, which is why this
	// went unnoticed — every remote OpenAI-compatible endpoint reached through
	// this client was accounted as zero.
...
// openAIStreamOptions carries the one streaming option Local Agent needs.
